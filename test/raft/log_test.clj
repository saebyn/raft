...
(facts "about log"
       ...
       (facts "about as-complete?"
              (fact "returns true if the last entry and at least one entry from the current term is in the log"
                    (let [raft (->
                                 (core/create-raft ..rpc.. ..store.. ..state-machine.. ..servers..)
                                 (append-entry ..term1.. ..command1.. nil nil nil))]
                      (as-complete? raft ..term1.. ..term1.. 0) => truthy))
              (fact "returns true if the last entry and at least one entry from the current term is in the log but are different entries"
                    (let [raft (->
                                 (core/create-raft ..rpc.. ..store.. ..state-machine.. ..servers..)
                                 (append-entry ..term1.. ..command1.. nil nil nil)
                                 (append-entry ..term2.. ..command1.. nil ..term1.. 0))]
                      (as-complete? raft ..term2.. ..term1.. 0) => truthy))
              (fact "returns false if the log is empty"
                    (let [raft (core/create-raft ..rpc.. ..store.. ..state-machine.. ..servers..)]
                      (as-complete? raft 0 0 0) => falsey))
              (fact "returns false if the last entry but no entries from the current term are in the log"
                    (let [raft (->
                                 (core/create-raft ..rpc.. ..store.. ..state-machine.. ..servers..)
                                 (append-entry ..term1.. ..command1.. nil nil nil))]
